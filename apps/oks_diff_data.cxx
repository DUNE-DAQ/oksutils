...
ReportCompositeParents(OksObject* o, int recursionLevel = -1)
{
  ...
  if (rcr_list) {
    for (std::list<OksRCR*>::const_iterator i = rcr_list->begin(); i != rcr_list->end(); ++i) {
      ...
      std::cout << "     " << rcr->obj << " ==\\" << rcr->relationship->get_name() << "\\==> " << o << std::endl;
...
printShortUsage(const char* appName, std::ostream& s)
{
  s << appTitle
    << "\n"
       "Usage: "
    << appName
    << " [-a] [-r] [-p] [-all] [-h] [-v] [-c class [-o object_id]] "
...
printUsage(const char* appName, std::ostream& s)
{
  s << appTitle
    << "\n"
       "Usage: "
    << appName
    << " [--attributes] [--relationships] [--composite-parents] [--all] [--help] [--version]"
       " [--class-name name_of_class [--object-id id_of_object]]\n"
    ...
    for (; class_iterator != classes.end(); ++class_iterator) {
      ...
      if (!(c2 = kernel2.find_class(className))) {
        ...
        std::cout << " DIFFERENCE " << ++classDiffsCount
                  << "."
                     " Database \'"
                  << dataFile2 << "\' does not have class \"" << className << "\", skip testing ...\n";
        continue;
      }
      ...
      if (*c1 != *c2) {
        ...
        std::cout << " DIFFERENCE " << ++classDiffsCount
                  << "."
                     " Class \""
                  << className << "\" is different in \"" << dataFile1 << "\" and \"" << dataFile2
                  << "\", skip testing ...\n";
        continue;
      }
      ...
      for (; object_iterator != objects1->end(); ++object_iterator) {
        ...
        if (!o2) {
          ...
          std::cout << " DIFFERENCE " << ++classDiffsCount
                    << "."
                       " There is no object "
                    << o1 << " in the \"" << dataFile2 << "\" database file.\n";
          continue;
        }
          ...
          for (std::list<OksAttribute*>::const_iterator ai = alist->begin(); ai != alist->end(); ++ai) {
            ...
            if (!(*d1 == *d2)) {
              ...
              if (printAttributeDifferences) {
                std::cout << "   " << classDiffsCount << "." << objDiffsCount << " The ATTRIBUTE \"" << a->get_name()
                          << "\" values differed:\n"
                             "    the value in the FILE \""
                          << dataFile1 << "\" is: " << *d1
                          << "\n"
                             "    the value in the FILE \""
                          << dataFile2 << "\" is: " << *d2 << std::endl;
              }
            }
          }
        }
          ...
          for (std::list<OksRelationship*>::const_iterator ri = rlist->begin(); ri != rlist->end(); ++ri) {
            ...
            if (!(*d1 == *d2)) {
              ...
              if (printRelationshipDifferences) {
                std::cout << "   " << classDiffsCount << '.' << objDiffsCount << " The RELATIONSHIP \"" << r->get_name()
                          << "\" values differed:\n"
                             "    the value in the FILE \""
                          << dataFile1 << "\" is: " << *d1
                          << "\n"
      ...
      for (; object_iterator != objects2->end(); ++object_iterator) {
        ...
        if (!o1 && (!object_id || *objUID == object_id)) {
          ...
          std::cout << " DIFFERENCE " << ++classDiffsCount
                    << "."
                       " There is no object "
                    << (*object_iterator).second << " in the \"" << dataFile1 << "\" database file.\n";
        }
      }
