...
printUsage(std::ostream& s)
{
  s << "Usage: oks_diff_schema [-a] [-r] [-m] [-sb] [-all] [-h | --help] [-v] schema_file1 schema_file2\n"
    ...
    << " - loaded file has no any class\n"
       "   1.."
    << __StartOfBadStatus__ << " - number of differences (is limited by the max possible value)\n";
}
  ...
  if (alist) {
    for (std::list<OksAttribute*>::const_iterator ai = alist->begin(); ai != alist->end(); ++ai) {
      ...
      if (!(a2 = c2->find_attribute(attributeName))) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no attribute \""
                  << attributeName << "\" defined in the \"" << schemaFile2 << "\" database file.\n";

        continue;
      }

      if (!(*a1 == *a2)) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " The ATTRIBUTE \"" << attributeName
                  << "\" differed";
        ...
        if (!printAttributeDifferences)
          ...
          if (a1->get_is_multi_values() != a2->get_is_multi_values())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      ...
                      << "      in the FILE \"" << schemaFile1 << "\" it is: \""
                      << (a1->get_is_multi_values() ? szMultiValues : szSingleValues) << "\"\n"
                      << "      in the FILE \"" << schemaFile2 << "\" it is: \""
                      << (a2->get_is_multi_values() ? szMultiValues : szSingleValues) << "\"\n";
          ...
          if (a1->get_is_no_null() != a2->get_is_no_null())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      ...
                      << "      in the FILE \"" << schemaFile1 << szItCan << (a1->get_is_no_null() ? szNot : "")
                      << szBeNull << "      in the FILE \"" << schemaFile2 << szItCan
                      << (a2->get_is_no_null() ? szNot : "") << szBeNull;
        }
      }
    }
  }
  ...
  if (alist) {
    for (std::list<OksAttribute*>::const_iterator ai = alist->begin(); ai != alist->end(); ++ai) {
      ...
      if (!(a1 = c1->find_attribute(attributeName))) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no attribute \""
                  << attributeName << "\" defined in the \"" << schemaFile1 << "\" database file.\n";
        continue;
      }
    }
  }
  ...
  if (rlist) {
    for (std::list<OksRelationship*>::const_iterator ri = rlist->begin(); ri != rlist->end(); ++ri) {
      ...
      if (!(r2 = c2->find_relationship(relationshipName))) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no " << szRELATIONSHIP << "\""
                  << relationshipName << "\" defined in the \"" << schemaFile2 << "\" database file.\n";

        continue;
      }

      if (!(*r1 == *r2)) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " The " << szRELATIONSHIP << "\""
                  << relationshipName << "\" differed";
        if (!printRelationshipDifferences)
          ...
          if (r1->get_low_cardinality_constraint() != r2->get_low_cardinality_constraint())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      ...
                      << "      in the FILE \"" << schemaFile1 << "\" it is: \""
                      << (r1->get_low_cardinality_constraint() == OksRelationship::Zero
                            ? szZero
                            : (r1->get_low_cardinality_constraint() == OksRelationship::One ? szOne : szMany))
                      << "\"\n"
                      << "      in the FILE \"" << schemaFile2 << "\" it is: \""
                      << (r2->get_low_cardinality_constraint() == OksRelationship::Zero
                            ? szZero
                            : (r2->get_low_cardinality_constraint() == OksRelationship::One ? szOne : szMany))
                      << "\"\n";
          ...
          if (r1->get_high_cardinality_constraint() != r2->get_high_cardinality_constraint())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      ...
                      << "      in the FILE \"" << schemaFile1 << "\" it is: \""
                      << (r1->get_high_cardinality_constraint() == OksRelationship::Zero
                            ? szZero
                            : (r1->get_high_cardinality_constraint() == OksRelationship::One ? szOne : szMany))
                      << "\"\n"
                      << "      in the FILE \"" << schemaFile2 << "\" it is: \""
                      << (r2->get_high_cardinality_constraint() == OksRelationship::Zero
                            ? szZero
                            : (r2->get_high_cardinality_constraint() == OksRelationship::One ? szOne : szMany))
                      << "\"\n";
          ...
          if (r1->get_is_composite() != r2->get_is_composite())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      ...
                      << "      in the FILE \"" << schemaFile1 << "\" it is "
                      << (r1->get_is_composite() ? szComposite : szWeak) << szReference << "      in the FILE \""
                      << schemaFile2 << "\" it is " << (r2->get_is_composite() ? szComposite : szWeak) << szReference;

          if (r1->get_is_exclusive() != r2->get_is_exclusive())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      << szTheRelationship << "Composite Exclusive Types differed: \n"
                      << "      in the FILE \"" << schemaFile1 << "\" it is "
                      << (r1->get_is_exclusive() ? szExclusive : szShared) << szReference << "      in the FILE \""
                      << schemaFile2 << "\" it is " << (r2->get_is_exclusive() ? szExclusive : szShared) << szReference;

          if (r1->get_is_dependent() != r2->get_is_dependent())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount
                      << szTheRelationship << "Composite Dependent Types differed: \n"
                      << "      in the FILE \"" << schemaFile1 << "\" it is "
                      << (r1->get_is_dependent() ? szDependent : szIndependent) << szReference << "      in the FILE \""
                      << schemaFile2 << "\" it is " << (r2->get_is_dependent() ? szDependent : szIndependent)
                      << szReference;
        }
      }
    }
  ...
  if (rlist) {
    for (std::list<OksRelationship*>::const_iterator ri = rlist->begin(); ri != rlist->end(); ++ri) {
      ...
      if (!(r1 = c1->find_relationship(relationshipName))) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no " << szRELATIONSHIP << "\""
                  << relationshipName << "\" defined in the \"" << schemaFile1 << "\" database file.\n";
  ...
  if (mlist) {
    for (std::list<OksMethod*>::const_iterator mi = mlist->begin(); mi != mlist->end(); ++mi) {
      ...
      if (!(m2 = c2->find_method(methodName))) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no METHOD \"" << methodName
                  << "\" defined in the \"" << schemaFile2 << "\" database file.\n";

        continue;
      }

      if (!(*m1 == *m2)) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " The METHOD \"" << methodName
                  << "\" differed";
        ...
        if (!printMethodDifferences)
          ...
          if (m1->get_description() != m2->get_description())
            std::cout << "    " << classDiffsCount << '.' << localDiffsCount << '.' << ++localAttrCount << szTheMethod
  ...
  if (mlist) {
    for (std::list<OksMethod*>::const_iterator mi = mlist->begin(); mi != mlist->end(); ++mi) {
      ...
      if (!(m1 = c1->find_method(methodName))) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no METHOD \"" << methodName
                  << "\" defined in the \"" << schemaFile1 << "\" database file.\n";
  ...
  try {
    ...
    for (; class_iterator1 != classes1.end(); ++class_iterator1) {
      ...
      if (!(c2 = kernel2.find_class(className))) {
        std::cout << "\n DIFFERENCE " << ++classDiffsCount << ". There is no class \"" << className << "\" in the \""
                  << schemaFile2 << "\" database file.\n";
      ...
      std::cout << std::endl
                << " DIFFERENCE " << ++classDiffsCount << '.' << " The CLASSES \"" << className << "\" differed:\n";
      ...
      if (c1->get_description() != c2->get_description()) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " The Class Descriptions differed: \n"
      ...
      if (c1->get_is_abstract() != c2->get_is_abstract()) {
        std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " The Class Abstraction differed: \n"
                  << "      in the FILE \"" << schemaFile1 << "\" it is: \""
                  << (c1->get_is_abstract() ? "abstract" : "not abstract") << "\"\n"
                  << "      in the FILE \"" << schemaFile2 << "\" it is: \""
                  << (c2->get_is_abstract() ? "abstract" : "not abstract") << "\"\n";
      }
        ...
        if (slist) {
          for (std::list<std::string*>::const_iterator spi = slist->begin(); spi != slist->end(); ++spi) {
            ...
            if (!c2->find_super_class(*spn)) {
              std::cout << "   " << classDiffsCount << '.' << ++localDiffsCount << " There is no SUPERCLASS \"" << *spn
                        << "\" defined in the \"" << schemaFile2 << "\" database file.\n";
    ...
    for (; class_iterator2 != classes2.end(); ++class_iterator2) {
      if (!kernel1.find_class(class_iterator2->second->get_name())) {
        std::cout << "\n DIFFERENCE " << ++classDiffsCount << ". There is no class \""
                  << class_iterator2->second->get_name() << "\" in the \"" << schemaFile1 << "\" database file.\n";
      }
    }
