...
usage()
{
  std::cout
    << "usage: oks_get_data\n"
       ...
       "                            if there is no -s, then for schema HEAD version of \'" TDAQ_CMT_RELEASE
       "\' or the explicit release\n"
       "       -s schema_version    get data for given schema version (extra -n or -e to get HEAD data version is "
       "required)\n"
...
main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(cp, "-h") || !strcmp(cp, "--help")) {
      ...
      if (data_version > 0) {
        std::cerr << "ERROR: use either explicit data version (parameter -n) or HEAD version for provided release name "
                     "(parameter -e ...)\n";
        usage();
        return EXIT_FAILURE;
      }
      ...
      if (++i == argc) {
        ...
        if (!release.empty()) {
          std::cerr << "ERROR: use either explicit data version (parameter -n) or HEAD version for provided release "
                       "name (parameter -e ...)\n";
          usage();
          return EXIT_FAILURE;
        }
...

    {
      std::unique_ptr<coral::ConnectionService> connection;
      {
        ...
        if (!tag.empty() || data_version <= 0 || schema_version < 0) {
          oks::ral::get_data_version(
            session.get(), working_schema, tag, schema_version, data_version, release.c_str(), verbose_level);
      ...
      if (OksClass* c = kernel.find_class(class_name)) {
        ...
        if (q.good()) {
          ...
          if (l && !l->empty()) {
            ...
            std::cerr << "ERROR: failed to find any object of class \"" << class_name << "\" satisfying query \""
                      << query << "\", exiting...\n";
          ...
          std::cerr << "ERROR: failed to parse query \"" << query << "\" for class class \"" << class_name
                    << "\", exiting...\n";
          unlink(schema_file.c_str());
          unlink(data_file.c_str());
          return (EXIT_FAILURE);
