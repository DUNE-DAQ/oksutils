...
usage()
{
  std::cout
    << "usage: oks_ls_data\n"
       ...
       "   -p partition_pattern show configurations or usage for partition names satisfying pattern (see syntax "
       "description below)\n"
       "   -S since_timestamp   show configurations archived or used since given moment (see timestamp format "
       "description below)\n"
       "   -T till_timestamp    show configurations archived before given moment (see timestamp format description "
       "below)\n"
...
main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(cp, "-h") || !strcmp(cp, "--help")) {
      usage();
      return (EXIT_SUCCESS);
    } else if (!strcmp(cp, "-b") || !strcmp(cp, "--base-data-only")) {
      ls_base_versions_only = true;
    } else if (!strcmp(cp, "-l") || !strcmp(cp, "--list-releases")) {
      list_db = true;
    } else if (!strcmp(cp, "-z") || !strcmp(cp, "--show-size")) {
      ls_get_size = true;
    } else if (!strcmp(cp, "-u") || !strcmp(cp, "--show-usage")) {
      ls_usage = true;
    } else if (!strcmp(cp, "-d") || !strcmp(cp, "--show-description")) {
      ls_description = true;
    }
  ...
  try {
    if (tmp_acrhived_since) {
      acrhived_since =
        oks::tz::posix_time_2_iso_string(oks::tz::str_2_posix_time(tmp_acrhived_since, tz_ptr, "archived-since"));
    }

    if (tmp_acrhived_till) {
      acrhived_till =
        oks::tz::posix_time_2_iso_string(oks::tz::str_2_posix_time(tmp_acrhived_till, tz_ptr, "archived-till"));
    }
  } catch (const std::exception& ex) {
  ...
  try {
    ...
    {
      std::unique_ptr<coral::ISessionProxy> session(
        oks::ral::start_coral_session(connect_string, coral::ReadOnly, connection, verbose_level));
      ...
      if (list_db) {
        ...
        if (schema_version == 0) {
          schema_version =
            oks::ral::get_head_schema_version(session.get(), working_schema, release.c_str(), verbose_level, "  ");
          if (schema_version == 0) {
        ...
        if (!sorted_by.empty()) {
          for (const char* p = sorted_by.c_str(); *p != 0; ++p) {
            ...
            switch (*p) {
              case 'v':
                query->addToOrderList("A.SCHEMAVERSION");
                query->addToOrderList("A.VERSION");
                break;
              case 'V':
                query->addToOrderList("A.SCHEMAVERSION DESC");
                query->addToOrderList("A.VERSION DESC");
                break;
              case 't':
                query->addToOrderList("A.TIME");
                break;
              case 'T':
                query->addToOrderList("A.TIME DESC");
                break;
              case 'u':
                query->addToOrderList("A.CREATEDBY");
                break;
              case 'U':
                query->addToOrderList("A.CREATEDBY DESC");
                break;
              case 'h':
                query->addToOrderList("A.HOST");
                break;
              case 'H':
                query->addToOrderList("A.HOST DESC");
                break;
              case 'p':
                query->addToOrderList("A.DESCRIPTION");
                break;
              case 'P':
                query->addToOrderList("A.DESCRIPTION DESC");
                break;
        ...
        rows.push_back(TableRow(' ',
                                ...
                                "Version",
                                "Release",
                                (tz_ptr ? "Date (local time)" : "Date (UTC)"),
                                "User",
                                "Host",
                                "Size",
                                "Description"));
        ...
        struct
        {
          long num;
          coral::ITable& table;
        } nums[] = { { 0, session->schema(working_schema).tableHandle("OKSOBJECT") },
        ...
        while (cursor.next()) {
          ...
          if (ls_description && !row["A.DESCRIPTION"].isNull())
            description = row["A.DESCRIPTION"].data<std::string>();
          std::string r;
          if (!row["B.RELEASE"].isNull())
            r = row["B.RELEASE"].data<std::string>();
          ...
          if (ls_usage) {
            ...
            if (!sorted_by.empty()) {
              for (const char* p = sorted_by.c_str(); *p != 0; ++p) {
                ...
                switch (*p) {
                  case 'v':
                    query->addToOrderList("RUNNUMBER");
                    break;
                  case 'V':
                    query->addToOrderList("RUNNUMBER DESC");
                    break;
                  case 't':
                    query->addToOrderList("TIME");
                    break;
                  case 'T':
                    query->addToOrderList("TIME DESC");
                    break;
                  case 'u':
                    query->addToOrderList("CREATEDBY");
                    break;
                  case 'U':
                    query->addToOrderList("CREATEDBY DESC");
                    break;
                  case 'h':
                    query->addToOrderList("HOST");
                    break;
                  case 'H':
                    query->addToOrderList("HOST DESC");
                    break;
                  case 'p':
                    query->addToOrderList("PARTITION");
                    break;
                  case 'P':
                    query->addToOrderList("PARTITION DESC");
                    break;
                }
              }
            }
            ...
            while (c.next()) {
              ...
              std::string p;
              if (!ar_row["PARTITION"].isNull())
                p = ar_row["PARTITION"].data<std::string>();
