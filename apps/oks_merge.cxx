...
ERS_DECLARE_ISSUE(oks_merge, BadCommandLine, "Bad command line: \"" << what << '\"', ((const char*)what))

ERS_DECLARE_ISSUE(oks_merge,
                  BadQuery,
                  "Failed to parse query \"" << query << "\" in class \"" << class_name << '\"',
                  ((const char*)query)((const char*)class_name))

ERS_DECLARE_ISSUE(oks_merge, BadClass, "Cannot find class \"" << class_name << '\"', ((const char*)class_name))
...
printUsage(const char* appName, std::ostream& s)
{
  s << appTitle
    ...
    << "\n"
       ...
       "    -r | --print-references N C1 C2 ... CX print objects referenced by found objects (can only be used with "
       "query), where:\n"
       "                                            * the parameter N defines recursion depth for referenced objects "
       "(> 0)\n"
       "                                            * the optional set of names {C1 .. CX} defines [sub-]classes for "
       "above objects\n"
...
main(int argc, char* argv[])
{
  ...
  try {
    ...
    if (query && !class_name) {
      ers::fatal(
        oks_merge::BadCommandLine(ERS_HERE, "Query can only be executed when class name is provided (use -c option)"));
      return __BadCommandLine__;
    }
