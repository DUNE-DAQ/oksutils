...
main(int argc, char** argv)
{
  ...
  try {
    desc.add_options()("files,f",
                       boost::program_options::value<std::vector<std::string>>(&files)->multitoken()->required(),
                       "oks files")("include,i",
                                    boost::program_options::value<std::vector<std::string>>(&include)->multitoken(),
                                    "preload oks files (to fix missing includes)")(
      "show-all-attributes,x", "show hidden attributes with values equal to defaults")("help,h", "Print help message");
