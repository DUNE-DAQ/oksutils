...
void
setPersonValues(OksObject* o,                    // OKS object describing person
...
void
printPerson(const OksObject* o // OKS object describing person
)
{
...
void
setEmployeeValues(OksObject* o,                    // OKS object that describes employee
...
void
printEmployee(const OksObject* o // OKS object that describes employee
)
{
...
void
setDepartmentValues(OksObject* o,    // OKS object that describes department
...
void
printDepartment(const OksObject* o // OKS object that describes department
)
{
...
main(int argc, char** argv)
{
  ...
  try {
    ...
    OksClass* Person = new OksClass("Person", "It is a class to describe a person", false, &kernel);

    Person->add(new OksAttribute(
      "Name", OksAttribute::string_type, false, "", "Unknown", "A string to describe person name", true));

    OksAttribute* PersonBirthday = new OksAttribute(
      "Birthday", OksAttribute::date_type, false, "", "2009/01/01", "A date to describe person birthday", true);

    Person->add(PersonBirthday);

    Person->add(new OksAttribute("Family_Situation",
    ...
    OksClass* Employee = new OksClass("Employee", "It is a class to describe an employee", false, &kernel);

    OksAttribute* EmployeeSalary = new OksAttribute(
      "Salary", OksAttribute::u32_int_type, false, "", "1000", "An integer to describe employee salary", false);

    OksRelationship* WorksAt = new OksRelationship("Works at",
                                                   "Department",
                                                   OksRelationship::One,
                                                   OksRelationship::One,
                                                   false,
    ...
    OksClass* Department = new OksClass("Department", "It is a class to describe a department", false, &kernel);

    OksAttribute* DepartmentName = new OksAttribute(
      "Name", OksAttribute::string_type, false, "", "Unknown", "A string to describe department name", true);

    OksRelationship* DepartmentStaff = new OksRelationship("Staff",
                                                           "Employee",
...

    {
      ...
      std::cout << "[QUERY]: Looking for persons were born between " << lowDate << " and " << highDate << " ...\n\n";
...

    {
      ...
      andExpression->add(new OksRelationshipExpression(
        WorksAt, new OksComparator(DepartmentName, new OksData(departmentName), OksQuery::equal_cmp)));

      OksQuery query(true, andExpression);

      // executes query

      std::cout << "[QUERY]: Looking for employee were born after " << aDate << " and works at department "
                << departmentName << " ...\n\n";
