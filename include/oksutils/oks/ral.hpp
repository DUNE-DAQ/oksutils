...
coral::ISessionProxy*
start_coral_session(const std::string& connect_string,
                    int mode,
...
int
get_head_schema_version(coral::ISessionProxy* session,
...
int
get_max_schema_version(coral::ISessionProxy* session,
                       const std::string& schema,
                       int verbose_level = 1,
                       const char* dx = "");
...
std::vector<int>
get_schema_versions(coral::ISessionProxy* session,
                    const std::string& schema,
                    const char* release = 0,
                    int verbose_level = 1,
                    const char* dx = "");
...
void
get_data_version(coral::ISessionProxy* session,
                 const std::string& schema,
 ...
 *  \param session         pointer on CORAL session
 *  \param schema          working schema
 *  \param schema_version  if >0 - new schema version; if 0, create new head version incrementing max defined version;
 * if <0, add classes to existing schema = MOD(schema_version)
 *  \param description     description for new schema
 *  \param verbose_level   level of verbose output (0 - none, 1 - normal, 2 - extended, 3 - debug, ...)
 */

void
put_schema(const OksClass::Map& classes,
           coral::ISessionProxy* session,
           const std::string& schema,
           int schema_version,
 ...
 *  \param data_version    new data version; if 0, create new head version incrementing max defined data version for
 * given schema
 *  \param base_version    if < 0, create new full version; if > 0, use given version as base for incremental
 * versioning; if = 0, use head version as base version
 *  \param description     description for new data
 *  \param inc_threshold   if it is above 0, then defines incremental version threshold size (data are not copied from
 * tempral tables on exceed)
 *  \param verbose_level   level of verbose output (0 - none, 1 - normal, 2 - extended, 3 - debug, ...)
 *
 *  \return Return number of relational tables rows to store the data; if it is negative, the new version is the same as
 * the base version. The schema_version and data_version parameters return versions to used to access stored data.
 */
...
void
tag_data(coral::ISessionProxy* session,
         const std::string& schema,
         int schema_version,
         int data_version,
         const std::string& tag,
...
void
create_archive_record(coral::ISessionProxy* session,
                      const std::string& schema,
                      int schema_version,
                      int data_version,
...
void
get_time_host_user(const coral::AttributeList& data,
                   std::string& time,
                   std::string& host,
                   std::string& user,
