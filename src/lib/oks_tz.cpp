...
oks::tz::str_2_posix_time(const std::string& in, boost::local_time::time_zone_ptr tz_ptr, const char* value)
{
  ...
  try {
    ...
    std::ostringstream text;
    text << "cannot parse " << value << " = \'" << in << "\': \"" << ex.what()
  ...
  if (tz_ptr) {
    try {
      boost::local_time::local_date_time lt(
        t.date(), t.time_of_day(), tz_ptr, boost::local_time::local_date_time::EXCEPTION_ON_ERROR);
      TLOG_DEBUG(1) << "Build zone\'s time \'" << in << "\' => \'" << lt.to_string() << "\' using \'"
                    << tz_ptr->to_posix_string() << '\'';
      t = lt.utc_time();
    } catch (std::exception& e) {
      std::ostringstream text;
      text << "cannot parse " << value << " = \'" << in << "\' in time zone \"" << tz_ptr->to_posix_string() << "\": \""
           << e.what() << '\"' << std::endl;
      throw std::runtime_error(text.str().c_str());
    }
  }
