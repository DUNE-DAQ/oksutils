...
static void
createInstances(unsigned long n,             // Number of instances
...
main(int argc, char** argv)
{
  ...
  for (int m = 1; m < argc; m++) {
    if (!strcmp(argv[m], "-nbind"))
      doNotBind = true;
    else if (!strcmp(argv[m], "-fast-exit"))
      fastExit = true;
    else if (!strcmp(argv[m], "-n"))
      totalNumberOfObjects = atol(argv[++m]);
    else if (!strcmp(argv[m], "-nlinks"))
      numberOfLinksPerSW = atol(argv[++m]);
    else if (!strcmp(argv[m], "-schema"))
      schemaFile = argv[++m];
    else if (!strcmp(argv[m], "-data"))
      dataFile = argv[++m];
    else
      Oks::warning_msg("data-generator") << "Unknown parameter: \"" << argv[m] << "\"\n";
  }

  if (!totalNumberOfObjects)
    totalNumberOfObjects = 2000;
  if (!numberOfLinksPerSW)
    numberOfLinksPerSW = 40;

  if (numberOfLinksPerSW < 10) {
    Oks::warning_msg("data-generator") << "Number of links per sw object can not be less 10, set minimal\n";
  ...
  numberOfEnvironment = totalNumberOfObjects - numberOfSWObjects - numberOfPrograms - numberOfSWResources -
                        numberOfProcess - numberOfComputers - numberOfApplications;
...

  {
    ...
    for (int i = 0; i < (int)numberOfPrograms; i++) {
      ...
      switch ((unsigned int)(rand() % 4)) {
        case 0:
          d_osType.SetValues("lynx", a);
          break;
        case 1:
          d_osType.SetValues("wnt", a);
          break;
        case 2:
          d_osType.SetValues("solaris", a);
          break;
        case 3:
          d_osType.SetValues("hpux", a);
          break;
      }
      ...
      try {
        ...
        Oks::error_msg("10") << "Failed to set attribute value of " << ProgramObjects[i] << ": " << ex.what()
                             << std::endl;
        return 10;
      }
    }
    ...
    for (int i = 0; i < (int)numberOfComputers; i++) {
      ...
      try {
        ...
        Oks::error_msg("11") << "Failed to set attribute value of " << ComputerObjects[i] << ": " << ex.what()
                             << std::endl;
        return 11;
      }
    }
  ...
  if (!doNotBind) {
    ...
    for (i = 0; i < (int)numberOfPrograms; ++i) {
      j = ((i < (int)numberOfSWObjects) ? i : (rand() % numberOfSWObjects));
    ...
    for (i = 0; i < (int)numberOfEnvironment; ++i) {
      ...
      if (j < 25 || !(j % 10)) /* needs for sw_object (32%) */
        SWObjectObjects[(rand() % numberOfSWObjects)]->AddRelationshipValue(SWObjectNeedsEnvironment_odi,
                                                                            EnvironmentObjects[i]);
      if ((j > 25 && j < 50) || !(j % 8)) /* needs for program  (34%) */
        ProgramObjects[(rand() % numberOfPrograms)]->AddRelationshipValue(ProgramNeedsEnvironment_odi,
                                                                          EnvironmentObjects[i]);
      else if (j > 50 || !(j % 7)) /* needs for application (57%) */
        ApplicationObjects[(rand() % numberOfApplications)]->AddRelationshipValue(ApplicationNeedsEnvironment_odi,
                                                                                  EnvironmentObjects[i]);
    ...
    if (numberOfEnvironment > 2500 || numberOfProcess > 1000 || numberOfPrograms > 2000)
      std::cout << std::endl;
  }
  ...
  for (i = 0; i < (int)numberOfPrograms; i++) {
    ...
    sprintf(buf, "/usr/local/dist/last/installed/g++/%s/bin/s%d", d_osType->data.ENUMERATION->c_str(), rand());
  ...
  std::cout << "Time to save data is "
            << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - tp).count() /
                 1000.
            << " ms" << std::endl;
