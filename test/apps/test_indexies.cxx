...
printUsage(const char* appName)
{
  std::cerr << appTitle << " Oks Kernel version " << OksKernel::GetVersion()
            << "\n"
               "Usage: "
            << appName
            << " [-h[elp]] [-v] SchemaFile DataFile ClassName AttributeName options [options]\n"
               "  Options:\n"
...
main(int argc, char** argv)
{
  ...
  if (!attributePnt) {
    std::cerr << "ERROR[4]: Can`t find attribute \"" << attributeName << "\" in class \"" << className
              << "\", exiting ...\n";
    return 4;
  }
    ...
    while (argc > 1) {
      ...
      if (olist) {
        ...
        for (; it != olist->end(); ++it) {
          ...
          if ((*it)->GetId().length() > idLength)
            idLength = (*it)->GetId().length();
