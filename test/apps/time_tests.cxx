...
struct CallTimeInfo
{
  CallTimeInfo(const char* s, double d)
    : str(s)
    , t(d)
  {
    ;
  ...
  {
    return (((this == &c) || (t == c.t && str == c.str)) ? true : false);
  };
...
std::ostream&
operator<<(std::ostream& s, const CallTimeInfo& c)
{
  double t = c.t;
...
#define TIME_TEST_STOP(STR)                                                                                            \
  ...
  double time_per_call =                                                                                               \
    std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - tp).count() / 1000.;      \
...
main(int argc, char** argv)
{
  ...
  { /* Construct kernel */
    ...
    d_Auth.data.LIST->push_back(new OksData("atdsoft"));
    ...
    size_t numberOfNewObjs = ((performance < 1000) ? 100 : (performance < 10000) ? 300 : 500);
    ...
    std::cout << " - create " << (numberOfSWObj2 - numberOfSWObj) << " instances of \'" << SWObjectClass->get_name()
              << "\' class\n"
                 " - create "
              << (numberOfPrObj2 - numberOfPrObj) << " instances of \'" << ProgramClass->get_name() << "\' class\n";
    ...
    std::cout << " - remove " << (numberOfSWObj2 - numberOfSWObj) << " instances of \'" << SWObjectClass->get_name()
              << "\' class\n"
                 " - remove "
              << (numberOfPrObj2 - numberOfPrObj) << " instances of \'" << ProgramClass->get_name() << "\' class\n";
...

    {
...

      {
        ...
        for (OksClass::Map::const_iterator ic = classes.begin(); ic != classes.end(); ++ic) {
          ...
          if (!c)
            std::cerr << "error in iterate-classes test\n";
        }
...

    {
      ...
      test_start_msg(countOfTests) << "Iterator over " << numberOfObjects << " objects of class \'"
                                   << EnvironmentClass->get_name() << "\'...";
...

      {
        ...
        for (OksObject::Map::const_iterator io = env_objs->begin(); io != env_objs->end(); ++io) {
          ...
          if (!o)
            std::cerr << "error in iterate-objects-of-class test\n";
        }
...

    {
...

      {
        ...
        for (OksObject::Set::const_iterator oi = all_objs.begin(); oi != all_objs.end(); ++oi) {
          OksObject* o = *oi;
          if (!o)
            std::cerr << "error in iterate-all-objects test\n";
        }
    ...
    test_start_msg(countOfTests) << " Query to find with range 1% from " << numberOfProgramObjs << " objects...";
...

    {
...

      {
        ...
        test_start_msg(countOfTests) << "Query with index to find with range 1% from " << numberOfProgramObjs
                                     << " objects...";
...

      {
        ...
        test_start_msg(countOfTests) << "Query with index to find with range 10% from " << numberOfProgramObjs
                                     << " objects...";
...

      {
        ...
        test_start_msg(countOfTests) << "Query with index to find with range 50% from " << numberOfProgramObjs
                                     << " objects...";
